...
class TurtleBot:
    ...
    def move2goal(self, goal_x, goal_y, goal_tol):
        ...
        while self.l2_distance(goal_pose) > tolerance :
            if abs(self.steering_angle(goal_pose) - self.yaw) >= tolerance:
                vel_msg.linear.x = 0
                vel_msg.angular.z = min(self.angular_velo(goal_pose, P = 0.3), 0.3)
            else:
                vel_msg.linear.x = min(self.linear_velo(goal_pose, P = 0.5), 0.3)
                vel_msg.angular.z = min(self.angular_velo(goal_pose, P = 0.3), 0.2)
            vel_msg.linear.y = 0
            vel_msg.linear.z = 0
    ...
    def move2goal_oriented(self, goal_x, goal_y, goal_theta, goal_tol):
        ...
        while self.l2_distance(goal_pose) > tolerance :
            if abs(self.steering_angle(goal_pose) - self.yaw) >= tolerance:
                vel_msg.linear.x = 0
                vel_msg.angular.z = min(self.angular_velo(goal_pose, P = 0.3), 0.4)
            else:
                vel_msg.linear.x = min(self.linear_velo(goal_pose, P = 0.5), 0.3)
                vel_msg.angular.z = min(self.angular_velo(goal_pose, P = 0.3), 0.2)
            vel_msg.linear.y = 0
